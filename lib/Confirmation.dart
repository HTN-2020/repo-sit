...
void getHttp(data) async {
  ...
  try {
    Response response =
        await Dio().post("https://rest.nexmo.com/sms/json", data: {
      "from": from,
      "to": numb,
      "text":
          "Thank you for using SitDown! You've booked space ${data[1] * 5 + data[2] + 1} for ${new DateFormat.yMMMd().add_jm().format(data[4])} to ${new DateFormat.yMMMd().add_jm().format(data[5])}.",
      "api_key": api_key,
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      body: Center(
        ...
        Container(
            margin: EdgeInsets.only(top: 60, bottom: 20, left: 13, right: 13),
            child: Text(
